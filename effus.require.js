...
var _requirejs = {
    ...
    Utils : {
        loadJSON : function(url,callback) {
            ...
            xobj.open('GET', url+( _requirejs.opts.nocache ? '?'+(new Date().getTime()) : '' ), true);
        ...
        iterationLoading : function(list,index,callback) {
            _requirejs.Utils.appendScript(list[index],function(){
                if (list.length > (parseInt(index)+1)) {
                    _requirejs.Utils.iterationLoading(list,parseInt(index)+1,callback);
                } else {
                    callback();
                }
            });
...
var requirejs = function(opts,allLoadedCallback) {
    ...
    _requirejs.Utils.loadJSON(_requirejs.opts.configSrc,function(json){
        ...
        if (typeof _requirejs.JSON[_requirejs.opts.project] == 'undefined')
            throw Error('There are no such project in config file: '+_requirejs.opts.project);
        var project = _requirejs.JSON[_requirejs.opts.project];
        if (typeof project.app == 'undefined')
            throw Error('Bad config file format: no such property "app" in definition of project '+_requirejs.opts.project);
        if (typeof project.require == 'undefined')
            throw Error('Bad config file format: no such property "require" in definition of project '+_requirejs.opts.project);
        if (typeof project.styles == 'undefined')
            throw Error('Bad config file format: no such property "styles" in definition of project '+_requirejs.opts.project);
